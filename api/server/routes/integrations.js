...
router.post('/gohighlevel/save', requireJwtAuth, async (req, res) => {
  try {
    ...
    await updateUserPluginAuth(userId, 'GHL_API_KEY', 'mcp_gohighlevel', apiKey);

    if (locationId) {
      await updateUserPluginAuth(userId, 'GHL_LOCATION_ID', 'mcp_gohighlevel', locationId);
    }
...
router.delete('/gohighlevel', requireJwtAuth, async (req, res) => {
  try {
    ...
    await deleteUserPluginAuth(userId, 'GHL_API_KEY', false, 'mcp_gohighlevel');
    await deleteUserPluginAuth(userId, 'GHL_LOCATION_ID', false, 'mcp_gohighlevel');
