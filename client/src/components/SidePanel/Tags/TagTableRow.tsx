...
import { DeleteTagButton, EditTagButton } from '~/components/Tags';
...
const TagTableRow: React.FC<TagTableRowProps> = ({ row, moveRow, position }) => {
  const ref = useRef<HTMLTableRowElement>(null);
  const mutation = useConversationTagMutation({ context: 'TagTableRow', tag: row.tag });
  ...
  const handleDrop = (item: DragItem) => {
    mutation.mutate(
      { ...row, position: item.index },
      {
        onSuccess: () => {
          showToast({
            message: localize('com_ui_tags_update_success'),
            severity: NotificationSeverity.SUCCESS,
          });
        },
        onError: () => {
          showToast({
            message: localize('com_ui_tags_update_error'),
    ...
    >
      ...
      <TableCell className="w-[20%] px-4 py-4">
        <div className="flex gap-2">
          <EditTagButton tag={row} tabIndex={0} />
          <DeleteTagButton tag={row.tag} tabIndex={0} />
        </div>
      </TableCell>
    </TableRow>
  );
