...
import TagNavItems from './TagNavItems';
...
const TagNav: FC<TagNavProps> = ({ tags, setTags, isSmallScreen }: TagNavProps) => {
  const localize = useLocalize();
  const { data } = useGetConversationTags();
  const conversation = useRecoilValue(store.conversationByIndex(0));
  const label = useMemo(
    () => (tags.length > 0 ? tags.join(', ') : localize('com_ui_tags')),
    [tags, localize],
  );
        ...
        <>
          <TooltipAnchor
            ...
            render={
              <MenuButton
                id="tag-menu-button"
                aria-label={localize('com_ui_tags')}
                className={cn(
          ...
          >
            {data && conversation && (
              <TagContext.Provider value={{ tags: data.filter((tag) => tag.count > 0) }}>
                <TagNavItems
